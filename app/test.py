...
# ----------------------------------------------------------
    ...
    with right:
        ...
        if st.checkbox("📉 Show Raw NaN count", key="clean_nan_count"):
            st.caption("Counts missing values for each column.")
            st.dataframe(df_clean.isna().sum())

        if st.checkbox("♻️ Show Raw duplicated rows", key="clean_duplicates"):
            st.caption("Counts duplicated rows in the clean dataset.")
            st.write(int(df_clean.duplicated().sum()))

        if st.checkbox(
            "⚠️ Show Raw rows with missing values", key="clean_missing_rows"
        ):
...
# ----------------------------------------------------------
    ...
    df_clean = df_clean.dropna(subset=["Animal"])  # minimum vital
