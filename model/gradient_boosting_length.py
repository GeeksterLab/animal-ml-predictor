...
FEATURES = ["Weight_kg"]   # Only predictor for Length
...
for species, df_subset in df.groupby("Animal"):
    ...
    # Save model
    save_model(model, f"gradient_boosting/gradient_boosting_length_{slugify(species)}.pkl")
    ...
    save_train(plt,
        f"gradient_boosting/gradientboosting_length_{slugify(species)}.png")

# ----------------------------------------------------------
# 💾 SAVE GLOBAL STATS
# ----------------------------------------------------------
df_stats = pd.DataFrame(stats)
save_stats(df_stats, "ML/gradient_boosting/stats_gradientboosting_length_per_species.csv")
...
save_feature(df_feature, "gradient_boosting/feature_importance_gradientboosting_length_per_species.csv")
